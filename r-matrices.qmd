---
title: "Day 4 morning, matrices"
author: "Liz Peterson"
format: html
editor_options: 
  chunk_output_type: console
---

## Create matrices in R

```{r}
# create sequence of values ----
my_values <- seq(from = 1, to = 10, by = 1)
my_values

# convert to matrix ----
my_matrix <- matrix(data = my_values, nrow = 2, ncol = 5, byrow = TRUE)
class(my_matrix)
```

## Incorrectly declared matrix sizes 

```{r}
matrix(data = my_values, nrow = 2, ncol = 3, byrow = TRUE)
matrix(data = my_values, nrow = 3, ncol = 4, byrow = TRUE)
# make sure to print out matrices so you know that it looks like what you want
```

## Let's do some math with matrices

```{r}
# scalar multiplication ----
4 * my_matrix

# let's create a second matrix
my_values2 <- seq(from = 21, to = 30, by = 1)
my_matrix2 <- matrix(data = my_values2, nrow = 2, byrow = TRUE) # don't need to specify columns, it will make it even

# add matrices ----
my_matrix + my_matrix2

# subtract matrices ----
my_matrix2 - my_matrix
```

```{r}
cats <- matrix(data = c(0, 4, 3, 1), nrow = 2, byrow = TRUE)
dogs <- matrix(data = c(6, -3, 0, 2), nrow = 2, byrow = TRUE)

# multiply matrices using dot product ----
cats %*% dogs
```

## Leslie matrix example

```{r}
# create the matrix ----
insect_leslie <- matrix(data = c(0, 0, 600, 0.2, 0, 0, 0, 0.08, 0), nrow = 3, ncol = 3, byrow = TRUE)

# initial starting conditions ----
insect_y0 <- c(12000, 700, 500) # from youngest to oldest age class

# pop at year 1 ----
insect_y1 <- insect_leslie %*% insect_y0

# pop at year 2 ----
insect_y2 <- insect_leslie %*% insect_y1

# pop at year 3 ----
insect_y3 <- insect_leslie %*% insect_y2
```

## Tree Leslie matrix example

```{r}
# create the matrix ----
tree_leslie <- matrix(data = c(0, 0, 5000, 0.1, 0, 0, 0, 0.06, 0.95), nrow = 3, ncol = 3, byrow = TRUE)

# initialize starting conditions ----
tree_y0 <- c(5000, 0, 0)

# pop at year 1 ----
tree_y1 <- tree_leslie %*% tree_y0

# pop at year 2 ----
tree_y2 <- tree_leslie %*% tree_y1
```

